...
export function jfire(env: Record<string, string>): Plugin {
  ...
  const send = (x: any) => {
    ...
    for (const c of clients) {
      try { c.write(d); } catch { clients.delete(c); }
    }
  };

  const getScanner = () => {
    if (s) return s;
    const apiKey = env.ALPACA_API_KEY || process.env.ALPACA_API_KEY || env.ALPACA_API_KEY_ID || process.env.ALPACA_API_KEY_ID;
    const apiSecret = env.ALPACA_API_SECRET || process.env.ALPACA_API_SECRET || env.ALPACA_API_SECRET_KEY || process.env.ALPACA_API_SECRET_KEY;
  ...
  return {
    ...
    configureServer(v: ViteDevServer) {
      v.middlewares.use('/api/status', (_, res) => {
        ...
        res.end(JSON.stringify(scanner?.status() || { connected: false, error: 'Missing Alpaca credentials' }));
      });

      v.middlewares.use('/api/stocks', (req, res) => {
        res.setHeader('Content-Type', 'application/json');
        const scanner = getScanner();
        const url = new URL(req.url || '', 'http://localhost');
        const symbol = url.searchParams.get('symbol');
        if (symbol && scanner) {
          res.end(JSON.stringify({ detail: scanner.getStockLiveDetail(symbol) }));
          return;
        }
        res.end(JSON.stringify(scanner?.payload() || { stocks: [], universeSize: 0, stocksTracked: 0 }));
      });
      ...
      v.middlewares.use('/api/news', (req, res) => {
        ...
        res.end(JSON.stringify({ articles: newsEngine?.getArticles(150) || [], status: newsEngine?.status() || { connected: false } }));
      });
      ...
      v.middlewares.use('/api/stream', (req, res) => {
        ...
        const scanner = getScanner();
        const newsEngine = getNewsEngine();
        res.write(`data: ${JSON.stringify({ type: 'connection_status', ...(scanner?.status() || {}) })}\n\n`);
        res.write(`data: ${JSON.stringify({ type: 'stocks', ...(scanner?.payload() || {}) })}\n\n`);
        if (newsEngine) {
          res.write(`data: ${JSON.stringify({ type: 'news_init', articles: newsEngine.getArticles(100) })}\n\n`);
          res.write(`data: ${JSON.stringify({ type: 'news_status', ...newsEngine.status() })}\n\n`);
        }
        const pingInterval = setInterval(() => { try { res.write(': ping\n\n'); } catch {} }, 15000);
        req.on('close', () => { clearInterval(pingInterval); clients.delete(res); });
      });
    },
    closeBundle() {
      s?.stop();
